...
const EquipmentTrackingPage: React.FC = () => {
  ...
  const filteredEquipment = useMemo(() => {
    return equipment.filter(item => item.station === currentStation);
  }, [currentStation]);
  ...
  // Calculate metrics based on filtered equipment
  const activeEquipment = filteredEquipment.filter(item => item.status === 'Active').length;
  const maintenanceEquipment = filteredEquipment.filter(item => item.status === 'Maintenance').length;
  const outOfServiceEquipment = filteredEquipment.filter(item => item.status === 'Out of Service').length;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 text-xl font-medium">
              Monitor and manage station equipment and vehicles
            </p>
