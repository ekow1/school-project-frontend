...
const RanksPage: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.initials.trim()) {
      newErrors.initials = 'Rank initials are required';
    ...
    // Gender validation: required only for junior ranks
            ...
            <button
              onClick={() => {
                ...
                setFormData({
                  ...
                  level: row.original.level?.toString() || '0',
                  group: (row.original.group as 'junior' | 'senior') || '',
  ...
  const filteredRanks = useMemo(() => {
    ...
    return transformedRanks.filter(
      rank =>
        ...
        rank.group?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rank.gender?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, transformedRanks]);
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({ 
                        ...formData, 
                        group: newGroup,
                        gender: newGroup === 'senior' ? '' as 'male' | 'female' | '' : formData.gender // Clear gender if switching to senior
                      });
                ...
                {(formData.group === 'junior' || editingRank?.group === 'junior') && (
                  <div>
