...
const IncidentReportsPage: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    return incidentReports.filter(incident => {
      if (activeTab === 'fire') {
        return incident.type === 'Fire';
      } else if (activeTab === '24hour') {
        return new Date(incident.reportedAt) > new Date(Date.now() - 24 * 60 * 60 * 1000);
      }
      return true;
    });
  }, [activeTab]);
  ...
  const criticalIncidents = incidentReports.filter(incident => incident.severity === 'Critical').length;
  const approvedIncidents = incidentReports.filter(incident => incident.status === 'Approved').length;
  const underReviewIncidents = incidentReports.filter(incident => incident.status === 'Under Review').length;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 text-xl font-medium">
              Comprehensive incident reporting and management system
            </p>
      ...
      {/* Incident Reports Table with Tabs */}
      <div className="bg-white border-2 border-gray-200 p-6 rounded-xl">
        <div className="flex items-center gap-3 mb-6">
          <div className="bg-gray-200 p-2 rounded-lg">
            <Shield className="w-6 h-6 text-gray-700" />
          </div>
          <h2 className="text-2xl font-black text-gray-900">Incident Reports</h2>
        </div>
